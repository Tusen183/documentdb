...
ListDatabasesWorker(void *fcinfoPointer)
{
	...
	for (int dbIndex = 0; dbIndex < numDatabases; dbIndex++)
	{
		...
		{
			ereport(ERROR, (errmsg("listDatabases worker input arrays cannot contain NULLs")));
		}
		...
		{
			for (int collectionIndex = startOffset; collectionIndex < endOffset; collectionIndex++)
			{
	...
	foreach(workerCell, workerResults)
	{
		...
		if (errorMessage != NULL)
		{
			...
			ereport(ERROR, (errcode(errorCode),
							...
							errdetail_log("Error running listDatabases %s", errorMessage)));
		}
