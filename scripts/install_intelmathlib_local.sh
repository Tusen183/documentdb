...
# Check if intelrdfpmath exists in scripts/dependencies
if [ -d "$scriptDir/dependencies/intelrdfpmath" ]; then
    echo "Found local intelrdfpmath"
    cd intelrdfpmath/LIBRARY
else
    echo "No local intelrdfpmath found, fetching from git..."
    git init
